...
async def telnyx_handler(telnyx_ws):
 ...
 async with deepgram_connect() as deepgram_ws:
  ...
  async def deepgram_receiver(deepgram_ws):
   ...
   # we will wait until the telnyx ws connection figures out the callsid
   ...
   # queues subscribed to the telnyx callsid
   async for message in deepgram_ws:
    for client in subscribers[callsid]:
     client.put_nowait(message)

   # once the telnyx call is over, tell all subscribed clients to close
  ...
  async def telnyx_receiver(telnyx_ws):
   print('telnyx_receiver started')
   # telnyx sends audio data as 160 byte messages containing 20ms of audio each
   # we will buffer 20 telnyx messages corresponding to 0.4 seconds of audio to improve throughput performance
   BUFFER_SIZE = 20 * 160
   ...
   async for message in telnyx_ws:
    try:
   ...
   # the async for loop will end if the ws connection from telnyx dies
  ...
  await asyncio.wait([
   ...
   asyncio.ensure_future(telnyx_receiver(telnyx_ws))
  ])
...
async def router(websocket, path):
 if path == '/client':
  ...
  print('telnyx connection incoming')
  await telnyx_handler(websocket)
